...
must_haves:
  truths:
    ...
    - "WorkingColorSpace enum is exported from lib.cinematic"
    - "Color functions including apply_lut are exported from lib.cinematic"
...
**2. Add ViewTransform and WorkingColorSpace to enums import:**
Add to the existing from .enums import block:
```python
from .enums import (
    LensType,
    LightType,
...
grep -c "WorkingColorSpace" lib/cinematic/__init__.py && \
...
grep -c '"apply_lut"' lib/cinematic/__init__.py && \
grep -c '"WorkingColorSpace"' lib/cinematic/__init__.py && \
...
```python
- color: Color management, LUT validation, compositor LUT application, and exposure lock
```
...
```python
    ...
    from lib.cinematic import (
        ColorConfig, LUTConfig, ExposureLockConfig, ViewTransform, WorkingColorSpace,
        set_view_transform, apply_color_preset,
        validate_lut_file, apply_lut, apply_exposure_lock,
    )
    ...
    if valid:
        lut_config = LUTConfig(name="kodak_2383", lut_path="luts/kodak_2383.cube", intensity=0.8)
...
from lib.cinematic import (
    ColorConfig, LUTConfig, ExposureLockConfig, ViewTransform, WorkingColorSpace,
    set_view_transform, apply_color_preset,
    get_current_color_settings, reset_color_settings, set_working_color_space,
    validate_lut_file, find_lut_path, load_lut_config,
    list_available_luts, apply_lut, remove_lut_nodes, get_active_luts,
    calculate_auto_exposure, apply_exposure_lock,
...
print(f'WorkingColorSpace values: {[e.value for e in WorkingColorSpace]}')
"
```
...
4. No circular imports or syntax errors
  ...
  <verify>
python3 -c "from lib.cinematic import ColorConfig, LUTConfig, ExposureLockConfig, ViewTransform, WorkingColorSpace, apply_lut; print('OK')"
  </verify>
  <done>
...
2. Verify __all__ list contains all color exports including apply_lut and WorkingColorSpace
...
- ViewTransform and WorkingColorSpace enums are exported from lib.cinematic
- All color preset loaders are exported (get_color_preset, get_technical_lut_preset, get_film_lut_preset)
- All color functions are exported (set_view_transform, apply_color_preset, validate_lut_file, apply_lut, etc.)
