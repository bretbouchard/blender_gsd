...
must_haves:
  truths:
    - "ColorConfig dataclass exists with view_transform, exposure, gamma, look properties"
...
1. **ColorConfig dataclass:**
   - view_transform: str = "AgX" (options: Standard, AgX, Filmic, Filmic Log, Raw, Log, ACEScg)
...
grep -c "class ExposureLockConfig" lib/cinematic/types.py
  ...
  <done>
ColorConfig, LUTConfig, and ExposureLockConfig dataclasses exist in types.py with to_dict and from_dict methods.
</done>
</task>
...
<task type="auto">
  ...
  <action>
Add a new ViewTransform enum to lib/cinematic/enums.py following the established pattern.
...
Add after the existing EasingType enum. This enum provides type-safe view transform selection matching Blender's OCIO options.
  </action>
  <verify>
grep -c "class ViewTransform" lib/cinematic/enums.py
  </verify>
  <done>
ViewTransform enum exists in enums.py with all required view transform options.
</done>
</task>
...
2. Verify enums.py contains ViewTransform enum
3. Verify preset_loader.py contains all color preset loader functions
4. Verify COLOR_CONFIG_ROOT is defined in preset_loader.py
</verification>

<success_criteria>
- ColorConfig dataclass exists with view_transform, exposure, gamma, look, display_device properties
