"""Test if Switch works with Compare node forcing evaluation."""
import bpy
import sys
import pathlib
...
print("=" * 60)
print("SWITCH WITH COMPARE NODE")
print("=" * 60)

# Use Compare node: Debug_Mode == True
compare = nk.n("FunctionNodeCompare", "Compare", 150, 0)
compare.operation = 'EQUAL'
compare.data_type = 'BOOLEAN'
nk.link(gi.outputs["Debug_Mode"], compare.inputs[0])
compare.inputs[1].default_value = True  # Compare with True

# Create Switch node
switch = nk.n("GeometryNodeSwitch", "Switch", 300, 0)
switch.input_type = 'MATERIAL'
nk.link(compare.outputs[0], switch.inputs["Switch"])
...
print(f"Modifier Debug_Mode = True")
