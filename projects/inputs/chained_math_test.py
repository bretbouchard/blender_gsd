"""Create a direct test to see actual evaluated values with correct scale."""
import bpy
...
# Math node for Z translation: 0 + 0.005 = 0.005m = 5mm
math_z = nt.nodes.new("ShaderNodeMath")
math_z.operation = "ADD"
math_z.location = (200, -100)
math_z.inputs[0].default_value = 0.0
math_z.inputs[1].default_value = 0.005  # 5mm
...
print("="*60)
print("DIRECT TRANSLATION TEST (1mm cube)")
print("="*60)

print(f"\nMath node inputs:")
print(f"  Input[0] = {math_z.inputs[0].default_value} (should be 0)")
print(f"  Input[1] = {math_z.inputs[1].default_value} (should be 0.005 = 5mm)")
print(f"  Input[2] = {math_z.inputs[2].default_value}")

print(f"\nCube vertices after translation (expected Z center at 5mm):")
z_vals = [v.co.z * 1000 for v in mesh.vertices[:4]]
for i, z_mm in enumerate(z_vals):
...
# Now test with chained math operations (like in our actual code)
print("\n" + "="*60)
...
# Create new node tree with chained math
nt = bpy.data.node_groups.new("ChainedTest", "GeometryNodeTree")
nt.interface.new_socket("Geometry", in_out="OUTPUT", socket_type="NodeSocketGeometry")
